...
import { riderChangePassword, updateRiderProfile, uploadProfilePhoto, submitInquiry } from '../../api';
import Input  from '../../components/Input';
import Button from '../../components/Button';
import { COLORS, SIZES, SHADOWS } from '../../theme';
import { vehicleLabel, errMsg } from '../../utils/helpers';

// ─── FAQ data (rider-specific) ────────────────────────────────────────────────
const FAQ_ITEMS = [
  {
...
export default function ProfileScreen({ navigation }) {
  ...
  const [panel,          setPanel]          = useState(null); // 'edit' | 'password' | 'support' | 'faq'
  ...
  // ─── Photo picker: works on both iOS & Android ────────────────────────────
  const handlePickPhoto = async () => {
    try {
      // Step 1: check existing permission first (avoids double-prompt on Android)
      const { status: existing } = await ImagePicker.getMediaLibraryPermissionsAsync();
      let finalStatus = existing;

      // Step 2: request only if not yet granted
      ...
      if (finalStatus !== 'granted') {
        return Toast.show({
          ...
          text2: 'Enable Photos permission in your device Settings.',
        });
      }

      // Step 3: open gallery — use string array for max SDK compatibility
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={styles.section}>
          <View style={styles.menuCard}>
            <MenuItem icon="log-out-outline" label="Sign Out" sublabel="See you again soon!" onPress={handleLogout} danger last />
          </View>
        </View>
