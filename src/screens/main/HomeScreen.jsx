      ...
      <View style={nm.sheet}>
        ...
        {loading
          ...
          : notifications.length === 0
            ? <View style={nm.ctr}><Ionicons name="notifications-off-outline" size={38} color="#D1D5DB" /><Text style={nm.none}>No notifications</Text></View>
            : <FlatList
                data={notifications}
                keyExtractor={(item, i) => item._id || String(i)}
                contentContainerStyle={{ padding: SIZES.lg, paddingBottom: 32 }}
                renderItem={({ item }) => (
                  <TouchableOpacity style={[nm.item, !item.read && nm.unread]} onPress={() => onMarkRead(item._id)} activeOpacity={0.8}>
                    <View style={nm.icon}><Ionicons name="megaphone-outline" size={16} color={BRAND} /></View>
                    <View style={{ flex: 1 }}>
...
export default function HomeScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      const [dR, oR] = await Promise.all([getDashboard(), getRiderOrders()]);
      const d = dR?.data?.data;
      if (d) { setDash(d); patchRider({ isOnline: d.isOnline, earnings: d.earnings, totalTrips: d.totalTrips, rating: d.rating }); }
      const all = oR?.data?.data?.orders || [];
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      Toast.show({ type: 'success', text1: next ? "You're Online!" : "You're Offline", text2: next ? 'Ready to receive orders' : 'Go online to earn' });
    } catch (e) { Toast.show({ type: 'error', text1: errMsg(e) }); }
  ...
  const handleAccept = async (id) => {
    ...
    } catch (e) { Toast.show({ type: 'error', text1: errMsg(e, 'Could not accept') }); load(); }
  ...
  if (loading && !dash) return (
    <SafeAreaView style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#F1F5FB' }} edges={['top']}>
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.topBar}>
        ...
        <TouchableOpacity style={s.bellBtn} onPress={() => { setNotifOpen(true); loadNotifs(); }}>
          <Ionicons name="notifications-outline" size={22} color="#374151" />
          {unread > 0 && <View style={s.bellDot}><Text style={s.bellDotTxt}>{unread > 9 ? '9+' : unread}</Text></View>}
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <LinearGradient colors={['#0A2F9A', '#1B4FD8', '#2563EB']} style={s.banner} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
          {/* Online toggle */}
          ...
          <View style={s.bannerStats}>
            ...
            <View style={s.bannerStatDiv} />
            <View style={s.bannerStat}>
              <Text style={s.bannerStatVal}>{fmtCurrency(balance)}</Text>
              <Text style={s.bannerStatLabel}>Balance</Text>
            </View>
          </View>
          ...
          {rider?.vehicle?.plate && (
            <View style={s.vehicleChip}>
              ...
              <Text style={s.vehicleChipTxt}>{rider.vehicle.plate}  ·  {rider.vehicle.model || vehicleLabel(rider.vehicle.type)}</Text>
            </View>
          )}
        </LinearGradient>
          ...
          <View style={s.activeWrap}>
            <View style={s.activeCard}>
              ...
              <View style={s.activeRoute}>
                ...
                <View style={s.activeRConn}>{[0,1,2,3].map(i=><View key={i} style={s.activeRDash}/>)}</View>
                <View style={s.activeRRow}>
              ...
              <TouchableOpacity style={s.manageBtn} onPress={() => navigation.navigate('ActiveOrder', { orderId: activeOrder._id })}>
          ...
          <View style={s.section}>
            <View style={s.sectionHdr}>
              <Text style={s.sectionTitle}>
                {isOnline
                  ? availableOrders.length > 0 ? `Nearby Orders (${availableOrders.length})` : 'Nearby Orders'
                  : 'Go Online to Earn'
                }
              </Text>
              {isOnline && availableOrders.length > 0 && (
                <View style={s.livePill}><View style={s.livePillDot}/><Text style={s.livePillTxt}>LIVE</Text></View>
              )}
            </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.qaRow}>
            {[
              ...
              <TouchableOpacity key={qa.label} style={s.qaItem} onPress={() => navigation.navigate(qa.screen)} activeOpacity={0.75}>
